    ...
    if (i < n) {
        ...
        for (int j=0; j<VERTICES; k = j++) {    // edge from vj to vk
            ...
            for (int ti=0; ti<tile_size; ti++) {
                ...
                #if use_method == 0
                if (  is_between(lpoints[ti].y, vj.y, vk.y) &&         //if p is between vj and vk vertically
                     (lpoints[ti].x < slope * (lpoints[ti].y-vj.y) + vj.x)
                ...
                int b = is_between(lpoints[ti].y, vj.y, vk.y);
                c[ti] += b && (lpoints[ti].x < vj.x + slope * (lpoints[ti].y - vj.y));
