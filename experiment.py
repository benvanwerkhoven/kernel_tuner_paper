...
def tune(algorithm, do_strategy):
    ...
    for method in test_methods:
        ...
        #test all methods 7 times because some methods are stochastic
        for i in range(7 if method else 1):
    ...
    for d in result_summary.values():
    ...
    for k in result_summary.keys():
        ...
        if k in results_db.keys():
            for sub_k, sub_v in result_summary[k].items():
                results_db[k][sub_k].append(sub_v)
        #add to db
        else:
